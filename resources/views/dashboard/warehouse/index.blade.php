<x-app-layout>
    <div class="relative overflow-hidden rounded-2xl bg-white shadow-md dark:bg-gray-800">
        ...
        <!-- FILTER FORM -->
        <div class="p-4">
            <form action="{{ route('dashboard') }}" method="GET" class="flex flex-wrap gap-2 items-center">
                <label class="text-sm">Threshold stok &lt;</label>
                <select name="threshold" class="rounded border px-2 py-1">
                ...
                <input type="date" name="date_start" class="rounded border px-2 py-1" value="{{ $selectedDateStart ?? '' }}" />
                <span class="text-sm">-</span>
                <input type="date" name="date_end" class="rounded border px-2 py-1" value="{{ $selectedDateEnd ?? '' }}" />

                <button type="submit" class="bg-[#225A97] text-white px-4 py-1 rounded">Filter</button>
                ...
                <div class="grid h-full grid-cols-4 justify-center gap-2 lg:gap-5 divide-x">
                    @if($lowStockItems->isEmpty())
                        <div class="col-span-4 p-6">
                            <div class="h-full flex items-center justify-center rounded-lg border border-gray-200 bg-gray-50 p-6">
                                <p class="text-gray-600 font-medium">Belum ada barang lagi yang stoknya rendah</p>
                            </div>
                        </div>
                    @else
                        @foreach($lowStockItems as $item)
                            <div class="col-span-1 p-2 md:p-5">
                                <div class="flex flex-col">
                                    ...
                                    <h1 class="text-sm text-gray-500">{{ $item->kode_barang ?? '-' }}</h1>
                                </div>
                                <div class="flex w-full flex-row items-end">
