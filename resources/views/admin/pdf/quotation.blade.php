...
<html lang="en">
    ...
    <head>
        <meta charset="utf-8">
        <title>{{ request('from') === 'sales_order' ? 'Sales Order' : 'Quotation' }} - Indonusa Jaya Bersama</title>
    ...
    <body class="m-0 flex flex-col items-center bg-slate-200 p-0 print:bg-white">
        ...
        <div class="page-break-preview bg-white shadow-md print:m-0 print:w-full print:shadow-none">
            ...
            <!-- INNER CONTENT -->
            <div class="content-wrapper z-1 relative w-full p-[1.27cm] text-[11pt] leading-[1.08] print:p-0" style="font-family: 'Tinos', serif;">
                ...
                <p class='mt-2 text-2xl text-[#2f5496]'>
                    <strong>{{ request('from') === 'sales_order' ? 'SALES ORDER' : 'QUOTATION' }}</strong>
                </p>
                            ...
                            @foreach ($requestOrder->supporting_images as $image)
                                ...
                                @if ($imgSrc)
                                    <div class="h-[90px] w-[90px] overflow-hidden border border-gray-300">
                                        <img src="{{ $imgSrc }}" alt="Supporting Image" class="h-full w-full object-cover" />
                                    </div>
                                @endif
                            @endforeach
                        </div>
                    ...
                    <table class="w-full border-collapse border border-black">
                        <thead class="border border-black bg-blue-900">
                            <tr>
                                ...
                                <th class="whitespace-nowrap border border-black px-2 py-1 text-center text-white">Kode Barang</th>
                        ...
                        <tbody>
                            ...
                            @forelse($requestOrder->items as $index => $item)
                                @php
                                    ...
                                    $unitDiscount = $displayHarga * (($item->discount_percent ?? 0) / 100);
                                ...
                                <tr>
                                    ...
                                    <td class="border px-2 py-1">
                                        {{ optional($item->barang)->goods_code ?? ($item->custom_product_code ?? '-') }}
                                    </td>
                                    <td class="border px-2 py-1">
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="border px-2 py-1 text-center">No items</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div class="break-inside-avoid">
                    ...
                    <div class="mt-8 text-[9pt]">
                        <p class="whitespace-pre-wrap" style="line-height: 1.6;">{{ $requestOrder->customer_notes }}</p>
                    </div>
                    ...
                    <div class="ml-auto mt-8 w-fit text-center text-[9pt]" style="page-break-inside: avoid; break-inside: avoid;">
                        ...
                        <div class="mt-2">
                            @if ($getPublicImageBase64('ttd.png'))
                                <img src="{{ $getPublicImageBase64('ttd.png') }}" alt="Signature" class="mx-auto h-20 object-contain" />
                            @endif
                        </div>
