...
<x-app-layout>
    <div class="inset-shadow-none dark:inset-shadow-gray-500 dark:inset-shadow-sm relative mb-5 flex justify-end overflow-hidden rounded-2xl bg-white shadow-md dark:bg-gray-800">
        <div class="p-4">
            {{-- Search --}}
            <form action="{{ route('goods-in-status.index') }}"
                method="GET"
                class="block pl-2">
    ...
    <div class="relative overflow-hidden rounded-2xl bg-white shadow-md dark:bg-gray-800">
        <div class="overflow-x-auto">
            ...
            <table id="DataTable" data-order='[]' class="hover w-full text-left text-sm text-gray-500 dark:text-gray-400">
                ...
                <tbody class="h-min-[300px]">
                        ...
                        <td class="w-fit px-4 py-3 text-right">
                            <div class="relative flex min-h-[40px] w-fit items-center justify-end">
                                ...
                                <div class="absolute left-0 z-10 flex flex-row overflow-hidden rounded-lg border border-gray-300 bg-white shadow-sm dark:border-gray-600 dark:bg-gray-700">
                                    ...
                                    {{-- Edit barang modal --}}
                                    <button class="edit-barang-btn group flex h-full cursor-pointer items-center justify-center bg-blue-700 p-2 text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                                        data-id="{{ $barang->id }}"
                                    ...
                                    <form action="{{ route('goods-in-status.destroy', $barang->id) }}"
                                        method="POST"
                    ...
                    <tr>
                        <td colspan="7"
                            class="text-center">Belum ada barang</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <nav class="flex flex-col items-start justify-between space-y-3 p-4 md:flex-row md:items-center md:space-y-0"
            ...
            <div class="flex items-center space-x-2">
                ...
                <form method="GET"
                    action="{{ route('goods-in-status.index') }}">
                    <input type="hidden"
                        name="search"
                        value="{{ request('search') }}">
                    <select name="perPage"
