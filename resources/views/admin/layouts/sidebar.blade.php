    ...
    <div
        ...
        <ul class="space-y-2">
                        ...
                        <li class="w-[75%]">
                            <a href="{{ route('delivery-orders.index') }}"
                                ...
                                <span
                                    ...
                                    Orders</span>
                                @php
                                    $deliveryOrderCount = \App\Models\Order::where(
                                        'status',
                                        'sent_to_warehouse',
                                    )->count();
                                @endphp
                                <span id="delivery-orders-notif-badge"
                                    class="{{ $deliveryOrderCount > 0 ? '' : 'hidden' }} ml-2 rounded-full bg-red-500 px-2 py-1 text-xs text-white">{{ $deliveryOrderCount }}</span>
                            </a>
                        </li>
            ...
            @if (in_array(auth()->user()->role, ['Sales']))
                <details
                    {{ request()->routeIs('sales.request-order.*') || request()->routeIs('sales.custom-penawaran.*') || request()->routeIs('customer.*') || request()->routeIs('sales.sales-order.*') ? 'open' : 'close' }}
                    class="">
                    <summary
                        ...
                        <svg width="30" height="30" viewBox="0 0 30 30" fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                            class="{{ request()->routeIs('sales.request-order.*') || request()->routeIs('sales.custom-penawaran.*') || request()->routeIs('customer.*') || request()->routeIs('sales.sales-order.*') ? 'text-white' : 'text-black dark:text-white' }} h-6 w-6 transition duration-75 group-hover:text-white">
            ...
            @if (in_array(auth()->user()->role, ['Supervisor']))
                ...
                <li>
                    <a href="{{ route('supervisor.custom-penawaran.index') }}"
                        ...
                        <span
                            class="{{ request()->routeIs('supervisor.custom-penawaran.*') ? 'text-white' : 'text-black dark:text-white' }} ml-3 group-hover:text-white">Approve
