<x-app-layout>
    ...
    <div class="relative overflow-hidden rounded-2xl bg-white shadow-md dark:bg-gray-800">
                            ...
                            @if (session('warehouse_filter_status') === 'defect')
                                <td class="px-4 py-3">
                                    @php
                                        $statusClass =
                                            [
                                                ...
                                                'ditolak_supervisor' => 'bg-red-50 text-red-800 inset-ring inset-ring-red-600',
                                    ...
                                    <div class="flex flex-col items-center justify-center gap-1">
                                        <span class="{{ $statusClass }} badge whitespace-nowrap">
                                            {{ $statusLabel }}
                                        </span>
                                    </div>
                                </td>
                                        ...
                                        <div class="absolute left-0 z-10 flex flex-row overflow-hidden rounded-lg border border-gray-300 bg-white shadow-sm dark:border-gray-600 dark:bg-gray-700">
                                            ...
                                            @elseif(session('warehouse_filter_status') === 'defect')
                                                @if ($barang->status_barang === 'ditolak_supervisor')
                                                    ...
                                                    {{-- Tombol Ajukan Ulang --}}
                                                    <button class="edit-barang-btn group flex h-full cursor-pointer items-center justify-center bg-blue-600 p-2 text-sm font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-500 dark:hover:bg-blue-600 dark:focus:ring-blue-800" title="Ajukan Ulang" data-id="{{ $barang->id }}" data-status="{{ $barang->status_listing }}" data-kode="{{ $barang->kode_barang }}" data-nama="{{ $barang->nama_barang }}" data-kategori="{{ $barang->kategori }}" data-stok="{{ $barang->parent->stok ?? 0 }}" data-stok-defect="{{ $barang->stok }}" data-satuan="{{ $barang->satuan }}" data-lokasi="{{ $barang->lokasi }}" data-harga="{{ $barang->harga }}" data-deskripsi="{{ $barang->deskripsi }}" data-gambar="{{ $barang->gambar }}" data-alasan="{{ $barang->alasan_pengajuan }}" data-is-re-submission="true">
                                                        ...
                                                        <span class="max-w-0 overflow-hidden text-nowrap opacity-0 transition-all duration-300 ease-in-out group-hover:max-w-xs group-hover:pl-2 group-hover:opacity-100">Ajukan
                                                            Ulang</span>
                                                    </button>
                                                @endif
                                            @endif
                                            <form action="{{ route('warehouse.destroy', $barang->id) }}" method="POST" style="display:inline;">
                                                ...
                                                @method('DELETE')
                                                @php
                                                    $isDefectFilter = session('warehouse_filter_status') === 'defect';
                                                    $confirmFunction = $isDefectFilter ? 'confirmCancel' : 'confirmDelete';
                                                    $btnLabel = $isDefectFilter ? 'Batalkan' : 'Hapus';
                                                @endphp
                                                <button type="button" class="group flex h-full cursor-pointer items-center justify-center bg-red-700 p-2 text-sm font-medium text-white hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-red-300 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900" onclick="{{ $confirmFunction }}(() => this.closest('form').submit())">
                                                    ...
                                                    <span class="max-w-0 overflow-hidden opacity-0 transition-all duration-300 ease-in-out group-hover:max-w-xs group-hover:pl-2 group-hover:opacity-100">{{ $btnLabel }}</span>
                                                </button>
                                            </form>
                                        </div>
                                    </div>
        ...
        @include('admin.warehouse.partials.warehouse-modal-show-note')
