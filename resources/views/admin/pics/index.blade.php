<x-app-layout>
    <div class="flex flex-col lg:h-[calc(100vh-112px)] overflow-hidden">
        <div
            ...
            <div class="mr-5 flex items-center space-x-3 md:w-auto">
                {{-- Search --}}
                <form action="{{ route('pics.index') }}" method="GET" class="block pl-2" data-realtime-table-search data-search-input="#topbar-search"
                ...
                <table id="" class="sortable hover w-full text-left text-sm text-gray-500 dark:text-gray-400">
                    <thead
                        ...
                        <tr>
                            ...
                            <th class="px-4 py-2 text-nowrap">No. HP</th>
                            ...
                            <th class="flex justify-end px-6 py-2 no-sort text-nowrap text-right">Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($pics as $pic)
                            <tr class="dark:border-gray-700">
                                ...
                                <td class="whitespace-nowrap px-6 py-2 text-right align-middle">
                                    <div class="flex justify-end">
                                        <div
                                            ...
                                            {{-- Edit Button --}}
                                            <button
                                                class="editPicsButton group flex h-full cursor-pointer items-center justify-center border-r border-blue-800 bg-blue-700 p-2 text-sm font-medium text-white transition-all duration-300 ease-in-out hover:bg-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-300 dark:border-blue-500 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-900"
            ...
            <nav id="pagination-nav" class="sticky bottom-0 z-20 flex flex-col items-start justify-between space-y-3 bg-white p-4 dark:bg-gray-800 md:flex-row md:items-center md:space-y-0"
                ...
                <div class="flex items-center space-x-2">
                    <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                        Menampilkan
                        <span
                            class="font-semibold text-gray-900 dark:text-white">{{ $pics->firstItem() ?? 0 }}-{{ $pics->lastItem() ?? 0 }}</span>
                        dari
                        <span
                            class="font-semibold text-gray-900 dark:text-white">{{ $pics->total() ?? $pics->count() }}</span>
                    </span>
    ...
    <!-- JS -->
    @vite(['resources/js/pics.js', 'resources/js/realtime-table-search.js', 'resources/js/table-sort.js'])
