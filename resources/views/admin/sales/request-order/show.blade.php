                ...
                <div class="lg:col-span-2">
                    ...
                    <div class="col-span-2 mb-6 flex items-center">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400">No. {{ $requestOrder->request_number }}
                            </p>
                        </div>
                    </div>
                    <div
                        ...
                        <div class="p-6">
                            <div class="mb-6 grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label class="mb-1 block text-sm font-semibold text-gray-700 dark:text-gray-300">ID
                                        Customer</label>
                                    <p class="text-gray-900 dark:text-white">{{ $requestOrder->customer_id ?? '-' }}</p>
                                </div>
                                <div>
                                    <label class="mb-1 block text-sm font-semibold text-gray-700 dark:text-gray-300">PIC
                                        (Sales)</label>
                                    <p class="text-gray-900 dark:text-white">{{ $requestOrder->sales->name ?? '-' }}
                                    </p>
                                </div>
                                <div>
                                    <label
                                ...
                                <div>
                                    <label
                                        class="mb-1 block text-sm font-semibold text-gray-700 dark:text-gray-300">Status</label>
                                    <div>
                                        @php
                            ...
                            <div class="flex flex-col items-end gap-4 p-6">
                                ...
                                @if (auth()->check() && auth()->user()->role === 'Supervisor' && $requestOrder->status === 'pending_approval')
                                    <div class="w-full flex-col space-y-2">
                                        <form method="POST"
                                            action="{{ route('supervisor.request-order.approve', $requestOrder->id) }}"
                                            class="block w-full">
                                            @csrf
                                            <button type="submit"
                                                class="flex w-full items-center justify-center rounded-lg bg-green-600 px-4 py-2 text-sm font-medium text-white hover:bg-green-700 focus:outline-none focus:ring-4 focus:ring-green-300 dark:focus:ring-green-800"
                                                onclick="return confirm('Setujui Request Order ini?')">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                    viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                    stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                                    class="lucide lucide-check mr-2 h-4 w-4">
                                        ...
                                        <!-- Reject with reason -->
                                        <button type="button"
                                            class="flex w-full items-center justify-center rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-4 focus:ring-red-300 dark:focus:ring-red-800"
                                            data-bs-toggle="modal" data-bs-target="#rejectModal">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                                class="lucide lucide-ban mr-2 h-4 w-4">
                                ...
                                <div class="flex w-full flex-wrap justify-end gap-2">
                                    ...
                                    @php
                                        ...
                                        if (
                                            auth()->check() &&
                                            in_array(auth()->user()->role, ['Supervisor', 'Admin'])
                                        ) {
                                            $canDownloadPdf = true;
                                        } elseif (auth()->check() && auth()->id() === $requestOrder->sales_id) {
                                            $canDownloadPdf = !in_array($requestOrder->status, [
                                                'pending_approval',
                                                'rejected',
                                            ]);
                                        }
