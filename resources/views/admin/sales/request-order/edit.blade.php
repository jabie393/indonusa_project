                    ...
                    <div class="card bg-light bg-card inset-shadow-none dark:inset-shadow-gray-500 dark:inset-shadow-sm mb-4 rounded-2xl shadow-sm">
                        ...
                        <div class="mb-8 grid grid-cols-1 gap-6 p-5 lg:grid-cols-2">
                            ...
                            <div class="col-span-2 flex flex-col">
                                ...
                                <select class="@error('customer_id') is-invalid @enderror {{ $dariCustomPenawaran ? 'cursor-not-allowed opacity-60' : '' }} block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-primary-600 focus:ring-primary-600 dark:border-gray-500 dark:bg-gray-600 dark:text-white dark:placeholder-gray-400"
                                        id="customer_id"
                                        name="customer_id"
                                                    ...
                                                    <div class="relative barang-dropdown-container">
                                                        ...
                                                        <button type="button" class="flex items-center justify-between w-full px-3 py-2 border border-subtle rounded-lg bg-surface text-body-sm text-on-surface-variant hover:border-primary transition-all dropdown-toggle-btn">
                                                            <span class="flex gap-2">
                                                                <span class="selected-barang-label text-nowrap">{{ $item->barang ? $item->barang->kode_barang : 'Pilih Barang' }}</span>
                                                            </span>
                                                                ...
                                                                <tbody class="divide-y divide-subtle/30 barang-options-body">
                                                                    @foreach ($goods as $b)
                                                                        <tr class="hover:bg-surface-container-high cursor-pointer barang-option-row"
                                                                            ...
                                                                            data-kategori="{{ $b->kategori }}"
                                                                            data-deskripsi="{{ $b->deskripsi ?? '' }}">
                                                                            <td class="px-4 py-3 text-body-sm font-semibold">{{ $b->kode_barang }}</td>
                                                                            <td class="px-4 py-3 text-body-sm">{{ $b->nama_barang }}</td>
                                                                            <td class="px-4 py-3 text-[12px] text-on-surface-variant">{{ $b->deskripsi ?? '-' }}</td>
                                                                            <td class="pr-4 text-primary text-right select-check-icon">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function calculateTotals() {
                ...
                document.querySelectorAll('.item-row').forEach(row => {
                    ...
                    row.querySelector('.harga-setelah-diskon-display').value = 'Rp ' + hargaSetelahDiskon
            ...
            document.addEventListener('click', function(e) {
                if (!e.target.closest('.barang-dropdown-container')) {
                    document.querySelectorAll('.dropdown-menu-container').forEach(m => m.classList.add('hidden'));
                }
            });
