                                ...
                                <div class="relative flex min-h-[40px] w-fit items-center justify-end">
                                    ...
                                    <div class="absolute left-0 z-10 flex flex-row overflow-hidden rounded-lg border border-gray-300 bg-white shadow-sm dark:border-gray-600 dark:bg-gray-700">
                                        ...
                                        @if (in_array($order->status, ['sent_to_warehouse', 'not_completed']))
                                            {{-- Approve --}}
                                            <button type="button" class="js-approve-order group flex h-full cursor-pointer items-center justify-center bg-green-700 p-2 text-sm font-medium text-white hover:bg-green-800 focus:outline-none focus:ring-4 focus:ring-green-300 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800" data-id="{{ $order->id }}" data-order-number="{{ $order->order_number }}" data-approve-url="{{ route('delivery-orders.approve', $order->id) }}">
