        ...
        <div class="flex flex-col justify-between gap-4 border-b border-gray-200 pb-4 dark:border-gray-700 md:flex-row md:items-center">
            ...
            <div class="flex flex-wrap gap-3">
                ...
                @if($procurement->status !== 'completed')
                    ...
                    <form action="{{ route('general-affair.procurement.force-complete', $procurement->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin memaksakan selesai (Force Complete) procurement ini? Sisa barang yang belum diterima akan dianggap batal, dan status Custom Quotation akan diubah ke Ready for Delivery.');">
                        @csrf
                        <button type="submit" class="flex items-center gap-2 rounded-lg bg-red-600 px-4 py-2 text-sm font-semibold text-white hover:bg-red-700 transition-all">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
