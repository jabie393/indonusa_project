    ...
    <div class="max-w-(--breakpoint-2xl) container mx-auto flex w-full flex-col gap-8 px-4 py-8 lg:flex-row">
                ...
                <div class="scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent flex max-h-[calc(100vh-300px)] flex-row gap-1 overflow-x-auto pb-4 lg:flex-col lg:overflow-y-auto lg:pb-0">
                    ...
                    @foreach ($allBrands as $brand)
                        <a href="{{ route('catalogs', ['brand' => $brand]) }}"
                           class="{{ request('brand') == $brand ? 'bg-[#0D223A] text-white shadow-md' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }} group flex items-center whitespace-nowrap rounded-lg px-4 py-2.5 text-sm font-medium transition-all duration-200">
                            {{ $brand }}
                        </a>
                    @endforeach
                </div>
        ...
        <div class="grid w-full grid-cols-1 gap-6 pb-20 sm:grid-cols-2 lg:w-[80%] xl:grid-cols-3">
            @forelse ($catalogs as $catalog)
                <div class="reveal reveal-up group flex flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm transition-all duration-300 hover:shadow-xl">
                    <!-- Cover Image -->
                    <div class="relative aspect-[3/4] overflow-hidden bg-slate-100">
                        ...
                        <div class="absolute inset-0 flex items-center justify-center gap-3 bg-[#0D223A]/40 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                            <a href="{{ asset('files/' . $catalog->catalog_file) }}"
                               target="_blank"
                               class="rounded-full bg-white p-3 text-[#0D223A] shadow-lg transition-colors hover:bg-[#0D223A] hover:text-white">
                                <svg class="h-6 w-6"
                                     fill="none"
                                     stroke="currentColor"
                                     viewBox="0 0 24 24">
                    ...
                    <div class="flex grow flex-col p-6">
                        ...
                        <div class="mt-auto flex items-center justify-between border-t border-slate-50 pt-4 text-slate-400">
                            <a href="{{ asset('files/' . $catalog->catalog_file) }}"
                               download
                               target="_blank"
                               class="flex items-center gap-2 text-xs font-black tracking-wider text-[#0D223A] hover:opacity-70">
                                <svg class="h-6 w-6"
                                     fill="none"
                                     stroke="currentColor"
                                     viewBox="0 0 24 24">
