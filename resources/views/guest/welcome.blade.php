...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="text-[#1b1b18] flex items-center lg:justify-center min-h-screen flex-col">
        <header class="bg-transparent w-full lg:max-w-4xl max-w-[335px] text-sm mb-6 mt-6 not-has-[nav]:hidden relative top-0 left-0">
            @if (Route::has('login'))
            <nav class="flex items-center justify-end gap-4">
                    <div class="flex flex-row">
                        <x-application-logo class="w-20 h-20 fill-current text-gray-500" />
                        <a href="/" class="text-2xl font-bold dark:text-[#EDEDEC]">INDONUSA JAYA BERSAMA</a>
                    </div>
                    <div>
                    @auth
                        <a
