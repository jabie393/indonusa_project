    ...
    <div class="max-w-(--breakpoint-2xl) container mx-auto flex w-full flex-col gap-10 px-4 py-8 lg:flex-row">
        ...
        <div class="w-full shrink-0 lg:w-[20%]">
            <div class="rounded-3xl border border-slate-100 bg-white p-8 shadow-xl lg:sticky lg:top-24">
                ...
                <div class="scrollbar-none flex max-h-[calc(100vh-320px)] flex-row gap-2 overflow-x-auto pb-4 lg:flex-col lg:overflow-y-auto lg:pb-0">
                    <a href="{{ route('order') }}" class="{{ !request('category') ? 'bg-[#0D223A] text-white shadow-lg' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }} whitespace-nowrap rounded-xl px-5 py-3 text-sm font-bold transition-all duration-300">
                        Semua Produk
                    </a>

                    @foreach ($allCategories as $kategori)
                        <a href="{{ route('order', ['category' => $kategori]) }}" class="{{ request('category') == $kategori ? 'bg-[#0D223A] text-white shadow-lg' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }} rounded-xl px-5 py-3 text-sm font-bold transition-all duration-300">
                            {{ $kategori }}
                        </a>
                    @endforeach
                </div>
        ...
        <div class="grid w-full grid-cols-1 gap-6 pb-20 sm:grid-cols-2 lg:w-[80%] xl:grid-cols-3">
            @forelse ($goods as $barang)
                <div class="group flex flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl">
                    <a href="{{ route('product.barang', $barang->id) }}">
                        <!-- Image Box -->
                        <div class="relative aspect-square overflow-hidden bg-slate-50">
                            @if (!empty($barang->gambar))
                                <img src="{{ url('files/' . $barang->gambar) }}" alt="{{ $barang->nama_barang }}" class="h-full w-full object-contain p-4 transition-transform duration-700 group-hover:scale-105">
                            @else
                                <div class="flex h-full items-center justify-center">
                                    <svg class="h-20 w-20 text-slate-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="flex grow flex-col p-8">
                            ...
                            <div class="mt-auto flex items-center gap-3 border-t border-slate-50 pt-6">
                                <!-- WhatsApp Button -->
                                <a href="https://wa.me/6281234567890?text={{ urlencode('Halo, saya ingin memesan ' . $barang->nama_barang . '. Apakah masih tersedia?') }}" target="_blank" class="flex flex-1 items-center justify-center gap-2 rounded-xl bg-[#225A97] py-4 text-xs font-bold text-white shadow-lg shadow-blue-200 transition-all hover:bg-[#0D223A]">
                                ...
                                <form action="{{ route('keranjang.tambah') }}" method="POST" class="inline">
                                    ...
                                    <button type="submit" class="rounded-xl bg-green-50 p-4 text-green-700 transition-all hover:bg-green-700 hover:text-white">
