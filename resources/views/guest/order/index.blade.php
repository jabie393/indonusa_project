<x-front-layout>
    <div
        class="duration-750 starting:opacity-0 flex min-h-[768px] w-full items-center justify-center bg-[#D9D9D9] opacity-100 transition-opacity lg:grow">
                    ...
                    @foreach ($goods as $barang)
                        <div
                                ...
                                <a href="{{ url('/product/' . $barang->id) }}">
                                    @if (!empty($barang->gambar))
                                        <img class="mx-auto h-full dark:hidden"
                                            src="{{ url('files/' . $barang->gambar) }}"
                                            alt="{{ $barang->nama_barang }}" />
                                        <img class="mx-auto hidden h-16 dark:block"
                                            src="{{ url('files/' . $barang->gambar) }}"
                                            alt="{{ $barang->nama_barang }}" />
                                    @else
                                        <img class="mx-auto h-full dark:hidden"
                            ...
                            <div class="pt-6">
                                ...
                                <p
                                    class="text-md font-extrabold leading-tight text-gray-900 dark:text-white md:text-2xl">
                                ...
                                <div class="mt-4 flex items-center justify-start gap-4">
                                    <a href="https://wa.me/6281234567890?text={{ urlencode('Halo, saya ingin memesan ' . $barang->nama_barang . ' dengan harga Rp ' . number_format($barang->harga, 0, ',', '.') . '. Apakah masih tersedia?') }}"
                                        target="_blank"
                                        class="inline-flex h-full items-center rounded-lg bg-[#225A97] px-3 py-2.5 text-xs font-medium text-white hover:bg-primary-800 focus:outline-none focus:ring-4 focus:ring-primary-300 md:px-5">
                                        Pesan
                                    </a>
